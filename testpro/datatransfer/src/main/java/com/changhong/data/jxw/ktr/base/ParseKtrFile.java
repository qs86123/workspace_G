package com.changhong.data.jxw.ktr.base;
...
public abstract class ParseKtrFile implements IParseKtrFile
{
     ...
     */
    public void parseKtrFile(DataSourceInfo[] tableInputAndTableOutput) throws ConnException
    {
        if (checkParame(tableInputAndTableOutput))
        {
            try
            {
                ...
                String fileName = new StringBuffer().append(tableInputAndTableOutput[0].getTableName())
                    .append("And")
     ...
     * @throws ConnException
     */
    private boolean checkParame(DataSourceInfo[] dataSource) throws ConnException
    {
        if (null == dataSource)
        {
        ...
        {
            fields = dataSourceInfo.getClass().getDeclaredFields();
            for (Field field : fields)
            {
                ...
                try
                {
                    if (null == field.get(dataSourceInfo))
                    {
