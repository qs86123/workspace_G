...
public class GifDecoder {
    ...
    protected void setPixels() {
        ...
        if(this.lastDispose > 0) {
            ...
            if(this.lastImage != null) {
                int[] var13 = ((DataBufferInt)this.lastImage.getRaster().getDataBuffer()).getData();
                System.arraycopy(var13, 0, dest, 0, this.width * this.height);
                if(this.lastDispose == 2) {
                    Graphics2D inc = this.image.createGraphics();
                    Color iline = null;
                    if(this.transparency) {
                        iline = new Color(0, 0, 0, 0);
                    } else {
                        iline = new Color(this.lastBgColor);
                    }

                    inc.setColor(iline);
                    inc.setComposite(AlphaComposite.Src);
                    inc.fill(this.lastRect);
                    inc.dispose();
                }
            }
        }
    ...
    public int read(String name) {
        ...
        try {
            ...
            if(name.indexOf("file:") < 0 && name.indexOf(":/") <= 0) {
                ...
                URL e = new URL(name);
                this.in = new BufferedInputStream(e.openStream());
            }
    ...
    protected int readBlock() {
        ...
        if(this.blockSize > 0) {
            int count1;
            try {
                for(boolean count = false; n < this.blockSize; n += count1) {
                    count1 = this.in.read(this.block, n, this.blockSize - n);
                    if(count1 == -1) {
                        break;
                    }
                }
