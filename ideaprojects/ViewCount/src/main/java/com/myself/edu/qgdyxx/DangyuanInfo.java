...
public class DangyuanInfo {
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(DangyuanInfo.class);

    public static void dangyuanInfo(String cookie, boolean isContact, String fileNAme) {
        //设置基本信息
        ...
        list.add(new BasicNameValuePair("page.pageSize", "10000"));
        if (isContact)
            list.add(new BasicNameValuePair("iscontact", "1"));
        //添加请求参数
        MyQuickCrawlUtil.INS.addPostParam(list);
        String content = MyQuickCrawlUtil.INS.dangyuanInfo(url, null, null, fileNAme);
